...
create index if not exists questions_dislikes_created_idx
  on public.questions (dislikes desc, created_at desc);
...
-- 4. 不喜歡去重表：一個 anon_id 對一題只能 -1 一次（DB 端強制）
...
-- 5. 開啟 Realtime（idempotent：判斷表是否已在 publication 內再加）
do $$
begin
  if not exists (
...
-- ⚠️ question_likes & question_dislikes 故意「不給任何 policy」→ anon 完全摸不到，
...
-- 8. RPC：原子地寫去重 row + bump dislikes
